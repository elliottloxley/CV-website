...
export default {
  ...
  methods: {
    ...
    getRadialPoint (radialPosition, magnitude = 1) {
      ...
      const x = (alteredRadius * Math.cos(radialPosition) + this.centerPosition[0])
      const y = (alteredRadius * Math.sin(radialPosition) + this.centerPosition[1])
    ...
    getRandomCoord (radius) {
      ...
      return {
        x: this.centerPosition[0] + radiusRandom * Math.cos(theta),
